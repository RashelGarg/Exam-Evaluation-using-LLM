...
def generate_rubric(client: anthropic.Anthropic, data: dict, total_marks: int) -> dict:
    """
...
def run_judge_with_consistency(
    ...
    for i in range(runs):
        result = run_single_judge(client, persona, data, rubric, total_marks)
...
def generate_feedback(client: anthropic.Anthropic, judge_results: dict, agg: dict, data: dict) -> dict:
...
def run_baseline(client: anthropic.Anthropic, data: dict, total_marks: int) -> int:
...
def run_bias_test(client: anthropic.Anthropic, data: dict, rubric: list, total_marks: int) -> dict:
    """
...
Reply ONLY with the rephrased answer, no extra text."""
    ...
    rephrase_resp = client.messages.create(
        model      = MODEL_NAME,
        max_tokens = 300,
        messages   = [{"role": "user", "content": rephrase_prompt}],
    )
    rephrased = rephrase_resp.content[0].text.strip()

    data_original  = data.copy()
    data_rephrased = data.copy()
    data_rephrased["student_answer"] = rephrased

    score_original  = run_single_judge(client, "moderate", data_original,  rubric, total_marks)["score"]
    score_rephrased = run_single_judge(client, "moderate", data_rephrased, rubric, total_marks)["score"]
...
def evaluate_one(
    ...
    rubric_result = generate_rubric(client, data, total_marks)
    rubric        = rubric_result["rubric"]

    print(f"  → Running Strict Judge  (×{SELF_CONSISTENCY_RUNS} runs)...")
    strict_result   = run_judge_with_consistency(client, "strict",   data, rubric, total_marks)

    print(f"  → Running Moderate Judge (×{SELF_CONSISTENCY_RUNS} runs)...")
    moderate_result = run_judge_with_consistency(client, "moderate", data, rubric, total_marks)

    print(f"  → Running Lenient Judge  (×{SELF_CONSISTENCY_RUNS} runs)...")
    lenient_result  = run_judge_with_consistency(client, "lenient",  data, rubric, total_marks)
    ...
    feedback = generate_feedback(client, judge_results, agg, data)

    print(f"  → Running baseline...")
    baseline_score = run_baseline(client, data, total_marks)

    bias_result = None
    if run_bias:
        print(f"  → Bias & stability test...")
        bias_result = run_bias_test(client, data, rubric, total_marks)
